"""
AgendamentoDispositivo Agendamentodadosde、Pesquisar、Aguardar.
"""
...
class CalendarManager:
    """
    AgendamentoDispositivo.
    """

    def __init__(self):
        self.db = get_calendar_database()
        # Tentativade  deJSONArquivoDados
        self._migrate_from_json_if_exists()

    def init_tools(self, add_tool, PropertyList, Property, PropertyType):
        """
        InicializandoAgendamento.
        """
        from .tools import (
            create_event,
            delete_event,
        ...
        add_tool(
            (
                ...
                "\nFeatures:\n"
                "- Shows time remaining until each event ('230')\n"
    ...
    def _migrate_from_json_if_exists(self):
        """
        de  deJSONarquivodados（SeExiste）
        """
        # PesquisarDiretórioEmde  JSONArquivo
        ...
        try:
            ...
            # SeIncapaz deDiretório，PesquisarDiretório
            user_cache_dir = get_user_cache_dir(create=False)
            json_file = user_cache_dir / "calendar_data.json"

        if os.path.exists(json_file):
            logger.info("deJSONDadosArquivo，ComeçarparaSQLite...")
            if self.db.migrate_from_json(json_file):
                # SucessoArquivo
                backup_file = f"{json_file}.backup"
                os.rename(json_file, backup_file)
                logger.info(f"DadosConcluído，ArquivoJápara: {backup_file}")
            else:
                logger.warning("DadosFalha，JSONArquivo")
